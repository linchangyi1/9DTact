...
class Estimator:
    def __init__(self, cfg):
        ...
        if model_name == 'Resnet':
            ...
            self.model = VisionTransformer(CONFIGS[model_type], img_size, num_classes=6).to(self.device)
        ...
        self.model.eval()
        # the first infer process takes much longer time than the latter, so just pass it before use it
        with torch.no_grad():
